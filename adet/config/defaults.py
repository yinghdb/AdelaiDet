...
# EmbedMask Options
# ---------------------------------------------------------------------------- #
_C.MODEL.EMBEDMASK = CN()

# the downsampling ratio of the final instance masks to the input image
_C.MODEL.EMBEDMASK.MASK_OUT_STRIDE = 4
_C.MODEL.EMBEDMASK.BOTTOM_PIXELS_REMOVED = -1

# if not -1, we only compute the mask loss for MAX_PROPOSALS random proposals PER GPU
_C.MODEL.EMBEDMASK.MAX_PROPOSALS = -1
# if not -1, we only compute the mask loss for top `TOPK_PROPOSALS_PER_IM` proposals
# PER IMAGE in terms of their detection scores
_C.MODEL.EMBEDMASK.TOPK_PROPOSALS_PER_IM = -1
_C.MODEL.EMBEDMASK.EMBED_DIM = 16
_C.MODEL.EMBEDMASK.PRIOR_MARGIN = 2.0
_C.MODEL.EMBEDMASK.MARGIN_REDUCE_FACTOR = 32.0
_C.MODEL.EMBEDMASK.MASK_LOSS_TYPE = "Lovasz"
_C.MODEL.EMBEDMASK.SAMPLE_IN_MASK = False
_C.MODEL.EMBEDMASK.MASK_LOSS_ALPHA = 1.0
_C.MODEL.EMBEDMASK.PROPOSAL_HEAD = "Conv3"
_C.MODEL.EMBEDMASK.MASK_TH = 0.5

_C.MODEL.EMBEDMASK.MASK_BRANCH = CN()
_C.MODEL.EMBEDMASK.MASK_BRANCH.OUT_CHANNELS = 128
_C.MODEL.EMBEDMASK.MASK_BRANCH.IN_FEATURES = ["p3", "p4", "p5"]
_C.MODEL.EMBEDMASK.MASK_BRANCH.CHANNELS = 128
_C.MODEL.EMBEDMASK.MASK_BRANCH.NORM = "BN"
_C.MODEL.EMBEDMASK.MASK_BRANCH.NUM_CONVS = 4
_C.MODEL.EMBEDMASK.MASK_BRANCH.SEMANTIC_LOSS_ON = False